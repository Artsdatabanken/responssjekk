...
let timeStamp
let current_kartlag
let current_kartlag_ul

function isURIencoded(url) {
  return url.includes('%')
}

function uriDecodeIfEncoded(url) {
  if (isURIencoded(url)) {
...
Object.keys(kartlag).forEach(key => {
  current_kartlag = kartlag[key]
  if (kartlag[key].wmsurl) {
    kartlag[key].wmsurl = uriDecodeIfEncoded(kartlag[key].wmsurl)
    const request = async () => {
      try {
  ...
  if (kartlag[key].underlag) {
    Object.keys(kartlag[key].underlag).forEach(ul => {
      ...
      current_kartlag_ul = kartlag[key].underlag[ul]
      if (kartlag[key].underlag[ul].legendeurl) {
        kartlag[key].underlag[ul].legendeurl = uriDecodeIfEncoded(kartlag[key].underlag[ul].legendeurl)
        const request = async () => {
          try {
            ...
            if (err.code === "ETIMEDOUT") {
              let kartlag_ul = kartlag[key].underlag[ul]
              kartlag_ul.status = "408 REQUEST TIMEOUT"
            }
          }
        }
        request()
