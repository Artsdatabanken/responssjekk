document.addEventListener("DOMContentLoaded", function(event) { 
  ...
  Object.keys(local_data).forEach(hovedlag => {
    ...
    if (local_data[hovedlag].underlag) {
      ...
      document.getElementById('hovedlag').innerHTML += '<div class="underlag" id="underlag">' + '<div style="display: inline">' + underlagKey + '</div>' + '<div style="display: inline">' + local_data[hovedlag].underlag[underlagKey]["tittel"] + '</div>' + '<div style="display: inline">' + local_data[hovedlag].underlag[underlagKey]["legendeurl"] + '</div>' + '</div>'
      })
